	...
	) 
{
    ...
    this.ColorsD = function () {
        ...
        for (var i = 0; i < colsPlusOne - 1; i++) {
            for (var j = 0; j < rowsPlusOne - 1; j++) {
                ...
                colorsData[c++] = t;
                colorsData[c++] = s;
                colorsData[c++] = t + sizeh;
                colorsData[c++] = s + sizew;
                colorsData[c++] = t;
                colorsData[c++] = s + sizew;
                colorsData[c++] = t;
                colorsData[c++] = s;
    ...
    this.drawSelf = function (ms, textureGrass, textureSand, textureRock)//绘制物体的方法
    {
        ...
        gl.bindTexture(gl.TEXTURE_2D, textureSand); //绑定纹理
        gl.uniform1i(gl.getUniformLocation(this.program, "sTextureSand"), 0); //将纹理送入渲染管线  	

        gl.activeTexture(gl.TEXTURE1); //设置使用的纹理编号-0
        gl.bindTexture(gl.TEXTURE_2D, textureGrass); //绑定纹理
        gl.uniform1i(gl.getUniformLocation(this.program, "sTextureGrass"), 1); //将纹理送入渲染管线  

        gl.activeTexture(gl.TEXTURE2); //设置使用的纹理编号-0
        gl.bindTexture(gl.TEXTURE_2D, textureRock); //绑定纹理
        gl.uniform1i(gl.getUniformLocation(this.program, "sTextureRock"), 2); //将纹理送入渲染管线 
